...
export async function getMailIds(
  ...
  function idsParser($: cheerio.CheerioAPI) {
    ...
    const maillist = mailDomList.map((_, item) => {
  ...
  for (let i = 2; i <= totalPage; i++) {
    await task.push(() =>
      getIdsByOnePageList(i).then(([, _ids]) => ids.push(..._ids)).then(() => {
        console.log(`已下载 第${i}页`);
      })
    );
  }
...
export async function getInvoiceDownloadUrls<T>(context: Context, getDownloadUrls: (context: Context, mailId: string) => Promise<T>) {
  const mailIds = await getMailIds(context);

  type MI = typeof mailIds[number];
  type DU = typeof getDownloadUrls extends (context: Context, mailId: string) => Promise<infer R> ? R : never;
  const infos: (MI & DU)[] = [];
  const task = creatTask(10);
  for (let id of mailIds) {
    await task.push(async () => {
      const urls = await getDownloadUrls(context, id.id).catch((e) => { console.log(e.message); throw e; });
      infos.push({ ...id, ...urls })
    })
  }
